...
export class AviationDetailedComponent implements OnInit {
  ...
  loadData(): void {
    ...
    this.excelDataService
      .getSheet('assets/data/raw/16_Aviation_Detailed.json')
      .subscribe({
        next: data => {
        ...
        error: error => {
          console.error('Failed to load aviation data from PostgreSQL.', error);
          this.error = 'Failed to load aviation data from PostgreSQL.';
          this.loading = false;
        }
      });
  }
