...
export class AdvancedTechnologyComponent implements OnInit {
  ...
  loadData(): void {
    ...
    this.excelDataService
      .getSheet('assets/data/raw/14_Advanced_Technology.json')
      .subscribe({
        next: data => {
        ...
        error: error => {
          console.error('Failed to load technology data from PostgreSQL.', error);
          this.error = 'Failed to load technology data from PostgreSQL.';
          this.loading = false;
        }
      });
  }
  ...
  get filteredTechnologies(): AdvancedTechnology[] { const q=this.search.toLowerCase().trim(); return this.technologies.filter((x,index)=>matchesSheetColumnFilters(this.sheetData,index,this.columnFilters)&&(!this.domainFilter||x.technologyDomain===this.domainFilter)&&(!this.statusFilter||x.status2025===this.statusFilter)&&(!q||[x.systemName,x.description,x.rangeCapability].some(v=>v.toLowerCase().includes(q)))); }
}
