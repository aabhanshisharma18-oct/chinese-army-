...
export class AirDefenceDetailedComponent implements OnInit {
  ...
  loadData(): void {
    ...
    this.excelDataService
      .getSheet('assets/data/raw/17_Air_Defence_Detailed.json')
      .subscribe({
        next: data => {
        ...
        error: error => {
          console.error('Failed to load air-defence data from PostgreSQL.', error);
          this.error = 'Failed to load air-defence data from PostgreSQL.';
          this.loading = false;
        }
      });
  }
  ...
  get filteredDefences(): AirDefenceDetailed[] { const q=this.search.toLowerCase().trim(); return this.defences.filter((x,index)=>matchesSheetColumnFilters(this.sheetData,index,this.columnFilters)&&(!this.typeFilter||x.typeCategory===this.typeFilter)&&(!this.statusFilter||x.status===this.statusFilter)&&(!q||[x.system,x.radar,x.exportName].some(v=>v.toLowerCase().includes(q)))); }
}
