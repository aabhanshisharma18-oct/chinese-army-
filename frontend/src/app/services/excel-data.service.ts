...
export class ExcelDataService {
  ...
  getAllSheets(): Observable<Record<string, unknown>> {
    if (this.useApi) {
      ...
      const apiEndpoints = {
        '00_INDEX': this.http.get(`${this.apiUrl}/workbook-index`),
        ...
        '15_References': this.http.get(`${this.apiUrl}/reference-sources`),
  ...
  private mapSheetToApiEndpoint(sheetName: string): string | null {
    const mapping: Record<string, string> = {
      ...
      '15_References': '/reference-sources',
